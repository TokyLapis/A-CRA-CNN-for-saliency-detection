...
class LOCAL_TIP_Net(nn.Module):
    def __init__(self):
        super(LOCAL_TIP_Net, self).__init__()
        ...
        self.mask5_o_a = nn.Sequential(nn.Conv2d(512, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())

        self.mask5_o_b = nn.Sequential(nn.Conv2d(512, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())
        ...
        self.mask4_o_a = nn.Sequential(nn.Conv2d(512, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())

        self.mask4_o_b = nn.Sequential(nn.Conv2d(512, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())
        ...
        self.mask3_o_a = nn.Sequential(nn.Conv2d(256, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())

        self.mask3_o_b = nn.Sequential(nn.Conv2d(256, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())
        ...
        self.mask2_o_a = nn.Sequential(nn.Conv2d(128, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())

        self.mask2_o_b = nn.Sequential(nn.Conv2d(128, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())
        ...
        self.mask1_o_a = nn.Sequential(nn.Conv2d(64, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())

        self.mask1_o_b = nn.Sequential(nn.Conv2d(64, 1, 3, padding=1),
                                       nn.BatchNorm2d(1),
                                       nn.Sigmoid())
        ...
        # -------------Bridge--------------
        # self.bridge1_a = nn.Sequential(nn.Conv2d(512, 512, 3, dilation=2, padding=2),##################
        #                                nn.BatchNorm2d(512),
        #                                nn.ReLU(inplace=True))
        # self.bridge2_a = nn.Sequential(nn.Conv2d(512, 512, 3, dilation=2, padding=2),
        #                                nn.BatchNorm2d(512),
        #                                nn.ReLU(inplace=True))
        # self.bridge3_a = nn.Sequential(nn.Conv2d(512, 512, 3, dilation=2, padding=2),
        #                                nn.BatchNorm2d(512),
        #                                nn.ReLU(inplace=True))

        # self.bridge1_b = nn.Sequential(nn.Conv2d(512, 512, 3, dilation=2, padding=2),
        #                                nn.BatchNorm2d(512),
        #                                nn.ReLU(inplace=True))
        # self.bridge2_b = nn.Sequential(nn.Conv2d(512, 512, 3, dilation=2, padding=2),
        #                                nn.BatchNorm2d(512),
        #                                nn.ReLU(inplace=True))
        # self.bridge3_b = nn.Sequential(nn.Conv2d(512, 512, 3, dilation=2, padding=2),
        #                                nn.BatchNorm2d(512),
        #                                nn.ReLU(inplace=True))
    ...
    def forward(self, input):
        ...
        hbg_a = self.upscore2(h5_a)
        hbg_b = self.upscore2(h5_b)

        att_a_ = self.attention_decoder5_a(hbg_a)
        M5o_a = self.mask5_o_a(att_a_)
        att_b_ = self.attention_decoder5_b(hbg_b)
        M5o_b = self.mask5_o_b(att_b_)

        M5_a = torch.cat((1*att_b_.mul(hbg_a), hbg_a),dim=1)

        M5_b = torch.cat((1*att_a_.mul(hbg_b), hbg_b),dim=1)
        ...
        M1o = torch.cat((M1o_a,M1o_b), dim=1)
        M2o = torch.cat((M2o_a, M2o_b), dim=1)
        M3o = torch.cat((M3o_a, M3o_b), dim=1)
        M4o = torch.cat((M4o_a, M4o_b), dim=1)
        M5o = torch.cat((M5o_a, M5o_b), dim=1)

        return M1, M2, M3, M4, M5, M1o, M2o, M3o, M4o, M5o
